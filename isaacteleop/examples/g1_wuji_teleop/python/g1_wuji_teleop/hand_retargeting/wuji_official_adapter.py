...
class WujiOfficialManusHandRetargeter:
    ...
    def ensure_retargeter(self, side: str) -> None:
        if side not in self._config_names:
            raise ValueError(f"Unsupported hand side: {side!r}")
        if side == "left" and self._retargeter_left is not None:
            return
        if side == "right" and self._retargeter_right is not None:
            return

        Retargeter = _import_upstream_retargeter()
        config_path = self._resolve_config_path(side)
        ...
        if side == "left":
            self._retargeter_left = retargeter
            self._left_joint_names = joint_names
        else:
            self._retargeter_right = retargeter
            self._right_joint_names = joint_names

    def _ensure_retargeters(self) -> None:
        self.ensure_retargeter("left")
        self.ensure_retargeter("right")
    ...
    def retarget_manus_skeleton(
        ...
        *,
        ...
        del device
        self.ensure_retargeter(side)
        started = time.perf_counter()
        hand_input = _as_float32_numpy(skeleton, (21, 3))

        if side == "left":
            qpos, ok, error = self._retarget_hand(
                "left",
                hand_input,
                self._retargeter_left,
                self._last_left,
            ...
            if ok:
                self._last_left = qpos.copy()
            return HandRetargetStepResult(
                frame_index=int(frame_index),
                left_joint_positions=qpos,
        ...
        if side == "right":
            ...
            if ok:
                self._last_right = qpos.copy()
            return HandRetargetStepResult(
                frame_index=int(frame_index),
                left_joint_positions=None,
                right_joint_positions=qpos,
                right_joint_names=self._right_joint_names,
                ok=ok,
                stale=not ok,
                error=error,
