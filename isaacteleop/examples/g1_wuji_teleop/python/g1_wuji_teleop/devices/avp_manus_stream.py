...
INPUT_PROFILE_CONFIG = "config"
INPUT_PROFILE_AVP_MANUS = "avp-manus"
INPUT_PROFILE_VR_MANUS = "vr-manus"
INPUT_PROFILE_CHOICES = (
    INPUT_PROFILE_CONFIG,
    INPUT_PROFILE_AVP_MANUS,
    INPUT_PROFILE_VR_MANUS,
)
...
def normalize_input_profile(value: str | None) -> str:
    raw = str(value or INPUT_PROFILE_CONFIG).strip().lower().replace("_", "-")
    aliases = {
        "default": INPUT_PROFILE_CONFIG,
        "yaml": INPUT_PROFILE_CONFIG,
        "avp": INPUT_PROFILE_AVP_MANUS,
        "apple-vision-pro": INPUT_PROFILE_AVP_MANUS,
        "vision-pro": INPUT_PROFILE_AVP_MANUS,
        "vr": INPUT_PROFILE_VR_MANUS,
        "controller": INPUT_PROFILE_VR_MANUS,
        "vr-controller": INPUT_PROFILE_VR_MANUS,
    }
    normalized = aliases.get(raw, raw)
    if normalized not in INPUT_PROFILE_CHOICES:
        raise ValueError(
            f"Unknown input profile {value!r}. Expected one of {list(INPUT_PROFILE_CHOICES)}"
        )
    return normalized


def apply_input_profile_config(
    config: Mapping[str, Any], input_profile: str | None = INPUT_PROFILE_CONFIG
) -> dict[str, Any]:
    profile = normalize_input_profile(input_profile)
    result = copy.deepcopy(dict(config))
    if profile == INPUT_PROFILE_CONFIG:
        return result

    avp_value = result.get("avp", {})
    if avp_value is None:
        avp_value = {}
    if not isinstance(avp_value, Mapping):
        raise ValueError("Config field 'avp' must be a mapping")
    avp = dict(avp_value)
    result["avp"] = avp

    if profile == INPUT_PROFILE_AVP_MANUS:
        avp["ee_pose_source"] = "wrist"
        avp["ee_hand_joint"] = "wrist"
        avp["controller_fallback_enabled"] = False
        return result

    avp["ee_pose_source"] = "controller"
    avp["controller_pose_source"] = "aim"
    avp["controller_fallback_enabled"] = False
    return result
...
def _controller_scalar(
    ...
    *,
    ...
    field_index = getattr(index_enum, field_name, None)
    if field_index is None:
        return default
    return float(group[field_index])
...
class TeleopMain:
    ...
    def from_config_file(
        ...
        *,
        input_profile: str | None = INPUT_PROFILE_CONFIG,
        oxr_handles: Any | None = None,
    ) -> "TeleopMain":
        config_path = Path(path).expanduser() if path else _default_config_path()
        config_path = config_path.resolve()
        config = apply_input_profile_config(_load_yaml(config_path), input_profile)
...
class NullTeleopMain:
    """Teleop-compatible fallback used when OpenXR devices are unavailable."""
    ...
    def __init__(
        ...
        *,
        ...
        _ = config, config_dir
        self._reason = str(reason or "OpenXR devices are unavailable")
        self._frame_count = 0
        self._announced = False

    @property
    def hand_joint_names(self) -> dict[str, list[str]]:
        return {side: [] for side in SIDE_NAMES}
    ...
    def __enter__(self) -> "NullTeleopMain":
        if not self._announced:
            print(
                "[TeleopMain] OpenXR device fallback active; "
                f"simulation continues with zero device inputs. reason={self._reason}",
                flush=True,
            )
    ...
    def step(self) -> dict[str, Any]:
        ...
        return {
            ...
            "head_pose": None,
            "ee_poses": {"left": None, "right": None},
            "hand_active": {"left": False, "right": False},
            "hand_skeletons": {"left": None, "right": None},
            "hand_joint_names": self.hand_joint_names,
            "hand_joint_positions": {"left": None, "right": None},
            "controller_inputs": {
                side: dict(ZERO_CONTROLLER_INPUTS) for side in SIDE_NAMES
            },
            "device_status": {
                "openxr_connected": False,
                "fallback": True,
                "reason": self._reason,
                "zero_pose": dict(ZERO_POSE),
            },
        }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config = apply_input_profile_config(_load_yaml(config_path), args.input_profile)
