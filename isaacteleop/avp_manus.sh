...
cd ~/workspace/IsaacTeleop

CXR_RUNTIME_SDK_VERSION=6.2.0 ./scripts/download_cloudxr_runtime_sdk.sh


cd ~/workspace/IsaacTeleop
conda activate isaacteleop

# 先删掉旧构建产物，避免误装旧 wheel
rm -rf build install
...
如果这里还报 GitHub clone/TLS 错误，先不要继续 build/install/pip。需要先解决网络，例如重试，或配置代理：
...
最后安装本地 wheel。因为你的环境已经有 numpy 等依赖，可以用 --no-deps 避免 --no-index 去找依赖：

pip install --force-reinstall --no-deps \
./install/wheels/isaacteleop-1.3+local-cp312-cp312-linux_x86_64.whl

验证：

python -c "from isaacteleop.cloudxr.runtime import runtime_version; print(runtime_version())"
...
export LD_LIBRARY_PATH="$CONDA_PREFIX/lib/python3.12/site-packages/nvidia/cu13/lib:$PWD/install/lib:${LD_LIBRARY_PATH:-}"
